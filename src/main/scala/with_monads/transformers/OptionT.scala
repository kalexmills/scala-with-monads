...
object OptionT {
  implicit def monadForOptionT[F[_]: Monad]: Monad[OptionT[F, *]] =
    new Monad[OptionT[F, *]] {
      def flatMap[A, B](fa: OptionT[F, A])(
          f: A => OptionT[F, B]
      ): OptionT[F, B] = fa.flatMap(f)
      def pure[A](x: A): OptionT[F, A] = OptionT(Monad[F].pure(Some(x)))
    }

  def fromOption[F[_]: Monad, A](o: Option[A]): OptionT[F, A] =
    OptionT(Monad[F].pure(o))
  def liftF[F[_]: Monad, A](o: F[A]): OptionT[F, A] = OptionT(o.map(Some(_)))
  def pure[F[_]: Monad, A](a: A): OptionT[F, A] =
    OptionT(Monad[F].pure(Some(a): Option[A]))
  def none[F[_]: Monad, A] = fromOption[F, A](None)
}
